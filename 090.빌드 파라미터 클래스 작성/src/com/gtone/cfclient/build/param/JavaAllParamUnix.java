...
public class JavaAllParamUnix implements IDynaParam {

	public String getParam(CommonDAO dao, HashMap param) throws Exception {
		 CacheResultSet  rs = (CacheResultSet)param.get("ALL_RESOURCE_RS");
		 ...
		 for(int i=0; rs.next() ;i++) 
		 {
			 ...
			 String path = rs.getString("RES_PATH");
			 /*if(isWindows)
			 {
				 path = StringHelper.replaceStr(path, "/", "\\");
			 }*/
			 String name = rs.getString("RES_NAME");
			 sb.append(path.substring(1)+name);
